			...
			<tbody>
				<c:forEach var="event" items="${events}">
					<tr>
						...
						<td><img height="150px"
							src="<c:url value='/geteventimage/${event.id}'/>" /></td>
						<td><input type="hidden" name="typeId"
							value="${event.typeId}" /> <input type="hidden" name="companyId"
							value="1" /> <input type="hidden" name="eventId"
							value="${event.id}" />
							<button class="deleteBtn btn btn-success"
								onclick="location.href = '<c:url value="/EventShow/${event.id}" />'">See
								More</button></td>
					</tr>
				</c:forEach>
			</tbody>
