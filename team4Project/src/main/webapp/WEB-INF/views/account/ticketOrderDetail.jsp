				    ...
				    <div class="col-xs-8">
				      ...
				      <div class="modal__container">
					      <div class="modal__content">
					            ...
					            <p style="text-align:left;font-size:18px">訂單編號:${ticketOrder.id}</p>
                  				<p style="text-align:left;font-size:16px">票券名稱:${event.eventName}</p>
                  				<p style="text-align:left;font-size:16px">總金額:${ticketOrder.totalCost}元</p>
                  				<c:choose>
	               					<c:when test="${ticketOrder.status==1}">
	               						<p style="text-align:left;font-size:16px">狀態:已付款</p>
	               						<p style="text-align:left;font-size:16px">訂單完成時間:${ticketOrder.validTime.toString().substring(0, 19)}</p>
	               					</c:when>
	               					<c:otherwise>
	               						<p style="text-align:left;font-size:16px">狀態:未付款</p>
	               						
	               					</c:otherwise>
	               				</c:choose>	
                  				<p style="text-align:left;font-size:16px">票券明細:</p>
                  					
                  				
                  				<table style="border:3px #cccccc solid;width:300px;text-align:center;" cellpadding="10" border='1'>
                  				<tr><td>票券名稱</td><c:forEach var="price" items="${priceList}">
	               					 <td>${price.name}</td>
	               					</c:forEach></tr>
	               					
                  				<tr><td>購買張數</td>
                  					<c:forEach var="ticketOrderDetail" items="${TicketOrderDetails}">
	               					 <td>${ticketOrderDetail.value}</td>
	               					</c:forEach>
	               				</tr>
	               				<tr><td>價格</td>
	               				<c:choose>
	               					<c:when test="${discount==1}">
	               						<c:forEach var="price" items="${dPriceList}">
	               						<td>${price}</td>
	               						</c:forEach>
	               					</c:when>
	               					<c:otherwise>
...
<script src='https://cdn.datatables.net/responsive/1.0.4/js/dataTables.responsive.js'></script>
      ...
      $(document).ready(function() {
    	  let eventJSON = ${event};
     	  let ticketOrderJSON = ${ticketOrder};
